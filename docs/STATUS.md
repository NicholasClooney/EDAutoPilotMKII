...
- `ShipControls` repeat semantics now send separate single taps with built-in pacing instead of collapsing repeats into one burst dispatch, and `AGENTS.md` now records that repeated actions must always be emitted as individually delayed presses.
