...
- `resume` is now implemented in `control_room.py` as a scrollable picker backed by structured saved history: Enter re-executes the selected command immediately, `e` reopens it for editing, and `*` on a haul entry saves or clears that haul setup as the explicit default used by later `haul` prompt flows
...
- **Control room persistence follow-up.** Local JSON-backed operator state and the `resume` picker are now implemented in `control_room.py`. Next live check: verify the modal UX, Enter-to-execute path, `e` edit path, and `*` explicit-default-haul path in the real `uv run python3 control_room.py` loop against Elite + CrossOver.
