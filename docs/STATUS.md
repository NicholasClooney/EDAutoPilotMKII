...
- Galaxy map destination flow re-validated after re-introducing the Odyssey-style result selection without poll/retry: `type_text("\n")` was no longer reliably committing the search field in live CrossOver runs, so the routine now submits search with a direct held Enter (default 0.2s). Live-validated flow: UI_Up + UI_Select, type text, held Enter, UI_Right, UI_Select, CamZoomIn, held UI_Select, fixed settle, single `NavRoute.json` check, close map. No poll/retry yet.
...
| Galaxy map destination | Done (live-validated) | `edap/routines/` — current live-validated Odyssey flow is UI_Up + UI_Select (focus search), type text, held Enter commit (`search_commit_hold_s`, default 0.2s), UI_Right, UI_Select, CamZoomIn, held UI_Select (`select_hold_s`, default 5s), fixed `plot_settle_s` sleep, single `NavRoute.json` check, then close map. No poll/retry yet. |
| Control room TUI | **Needs live validation** | `control_room.py` — Textual TUI with ship status, activity log, and market panels; dispatches `dock`/`undock`/`jump`/`buy`/`sell`/`haul`/`dest` routines from the input bar; `dest <system>` / `set_dest <system>` wired to `set_gal_map_destination`; `sell` with no args iterates full cargo manifest; `haul [commodity]` prompts step-by-step for commodity, buy station, sell station; sell station defaults to current docked station; enforces start-at-sell-station precondition; `verbose on/off` toggles key press logging; command history (up/down arrows); `commands` lists supported commands; `help [command]` explains them in plain English; Ctrl-C/Ctrl-D now start the unified shutdown path, cancel the active routine worker first when one is running, then exit; input bar auto-focused on launch; `market filter <name>` sets filter (title-cased); `market` / `market clear` clears; `market lock/unlock`. Local tests cover command/help dispatch and shutdown ordering. Still needs live validation in the real `uv run python3 control_room.py` flow to confirm the noisy extra-interrupt/stop-exception bug is gone end-to-end. |
