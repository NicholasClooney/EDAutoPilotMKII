...
| Galaxy map destination | Done | `edap/routines.py` — `set_gal_map_destination` opens map, types system name, picks search result, plots with `CamZoomIn + UI_Select(held)`, verifies `NavRoute.json`, retries on mismatch; wired to `run_routine.py` as `--routine set_gal_map_destination --destination <SYSTEM>`; live-validated with "Colonia" |
