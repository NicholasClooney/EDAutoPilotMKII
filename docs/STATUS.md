...
- Stable release `v1.7.1` now packages the docked-location bootstrap fix, configured-title hyperspace-arrival TTS fix, nested control-config sections, digit-aware location TTS normalization plus `speak.py` smoke-test flags, and the tightened docs/README operator surface.
