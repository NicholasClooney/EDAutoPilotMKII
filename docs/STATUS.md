...
- **ETA-based supercruise disengage (7-second rule).** OCR the ETA countdown displayed next to the destination reticle during supercruise approach. Disengage proactively at ~7s remaining rather than reacting to the "TO DISENGAGE" popup. Tighter, more consistent drops. Requires pytesseract on a cropped orange-filtered region around the reticle label. The left-panel distance/speed readout is a fixed position and may be easier to OCR than the floating reticle label.
