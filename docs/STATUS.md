...
- the Control Room screenshot asset path is reserved, but no real tracked screenshot is in the repo yet.
...
| Control room TUI | Done | `control_room.py` — Textual TUI with ship status, activity log, and market panels; the left pane is wider, and the activity log is back on `RichLog` so the original rich colors/highlighting are preserved. `dest <system>` / `set_dest <system>` prompts for galaxy-map settle seconds (Enter = config default, unless editing a saved history entry). `haul [commodity]` prompts step-by-step for commodity, buy station, sell station, systems, galaxy-map settle seconds, and haul docking timeout seconds, and can reuse one explicit saved default haul profile from the local control-room state file across restarts. `replay` (alias `history`) opens a scrollable recent-command browser in the activity-pane area (cross-session history, default retention 20) where Enter re-executes, `e` reopens for edit, and `*` marks or clears that haul entry as the saved default; haul edit restores the structured prompt flow. Sell station defaults to current docked station; start-at-sell-station precondition still enforced. `boost` now fires three unconditional boosts; `escape` remains the mass-lock-aware `Status.json` escape routine. `verbose on/off` toggles key press logging; command history (up/down arrows); `commands` lists supported commands; `help [command]` explains them in plain English; Ctrl-C/Ctrl-D now cancel the active routine worker and keep control room open when a routine is running, and only trigger app shutdown when idle; input bar auto-focused on launch; `market filter <name>` sets filter (title-cased); `market` / `market clear` clears; `market lock/unlock`. Sell-all now falls back to `Cargo.json` if no fresh `Cargo` journal event has populated the live manifest yet. Local tests cover command/help dispatch, interrupt/shutdown ordering, config defaults, explicit default-haul reuse, history trimming, control-room state round-tripping, and the split boost-vs-escape command behavior. Interrupt handling is live-validated in the real `uv run python3 control_room.py` flow; `replay` / explicit-default-haul UX still need live validation. |
...
- **Repo cleanup and positioning.** Plan 0006 is implemented: macOS-first / CrossOver support is now explicit in the README, Control Room is the primary entrypoint, legacy Windows-era files are archived under `archive/legacy-windows/`, scratch probes live under `tools/scratch/`, and low-level utility detail moved into `docs/getting-started/`, `docs/operators/`, and `docs/diagnostics/`. Remaining follow-up: add a real tracked Control Room screenshot under `docs/assets/`.
