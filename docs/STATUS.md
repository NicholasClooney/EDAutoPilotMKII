...
- `undock` now exists as a journal-driven routine. It sends `UI_Back x10`, `HeadLookReset`, a single `UI_Down` tap, and `UI_Select` to trigger launch, waits for the `Undocked` journal event (configurable timeout, default 30s), then waits for a stricter post-launch in-space confirmation event (default 180s; currently `Location` with `Docked == false` or an equivalent flight-state event) before sending `SetSpeed100` and a single boost. The legacy `SetSpeedZero` calls between launch confirm and undock completion were dropped — the ship is still in the docking bay at that point and throttle state is irrelevant. Discrepancy noted in `docs/plans/0003-journal-driven-routines.md`.
...
| Undock sequencing | Done | `edap/routines/` — menu walk (UI_Back x10, HeadLookReset, UI_Down, UI_Select), waits for `Undocked`, then for a post-launch in-space confirmation event before sending `SetSpeed100` and one boost; pre-change flow live-validated, follow-up behavior still needs live validation |
...
| Control room TUI | **Needs live validation** | `control_room.py` — Textual TUI with ship status, activity log, and market panels; the left pane is wider, and the activity log is back on `RichLog` so the original rich colors/highlighting are preserved. `dest <system>` / `set_dest <system>` prompts for galaxy-map settle seconds (Enter = config default). `haul [commodity]` prompts step-by-step for commodity, buy station, sell station, systems, galaxy-map settle seconds, and haul docking timeout seconds (Enter = config default). Sell station defaults to current docked station; start-at-sell-station precondition still enforced. `verbose on/off` toggles key press logging; command history (up/down arrows); `commands` lists supported commands; `help [command]` explains them in plain English; Ctrl-C/Ctrl-D now cancel the active routine worker and keep control room open when a routine is running, and only trigger app shutdown when idle; input bar auto-focused on launch; `market filter <name>` sets filter (title-cased); `market` / `market clear` clears; `market lock/unlock`. Sell-all now falls back to `Cargo.json` if no fresh `Cargo` journal event has populated the live manifest yet. Local tests cover command/help dispatch, interrupt/shutdown ordering, and the Cargo.json sell-all fallback. Still needs live validation in the real `uv run python3 control_room.py` flow to confirm the noisy extra-interrupt/stop-exception bug is gone end-to-end. |
...
- **Control room live follow-up.** Interrupt semantics and command discovery/help are now implemented in `control_room.py`, with focused tests. The remaining task is live validation in the real terminal/game loop: confirm that `Ctrl-C` during an active routine cancels the worker without exiting the TUI, and that `Ctrl-C` when idle exits without needing extra interrupts or leaving a noisy Python stop/interrupt exception behind in the terminal.
- **High-priority TODO: persistent local operation settings.** `control_room.py` currently forgets haul prompts after restart. Add a local user settings/config file for repeated operation inputs such as haul commodity, buy/sell stations, systems, and timing overrides so restart-test loops do not require re-entering the same parameters.
