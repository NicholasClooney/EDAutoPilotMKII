...
- Market sell and the control-room market panel now keep zero-demand rows visible when the station still exposes a sell price, so cargo like `Food Cartridges` is no longer skipped just because `Market.json` reports `DemandBracket = 0`.
