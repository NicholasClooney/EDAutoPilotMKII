...
| Status.json reader | Done | `edap/status.py` — `StatusFlags.from_int()` decodes all 32 Flags bits into named boolean fields; `ShipStatus` holds flags, pips, fuel, cargo, legal state, and balance; `read_status(journal_dir)` returns a `ShipStatus` or `None`. Not yet wired into any routine or the control room. |
...
- **Control room persistence follow-up.** Local JSON-backed operator state and the `replay` picker are now implemented in `control_room.py`. Next live check: verify the modal UX, Enter-to-execute path, `e` edit path, and `*` explicit-default-haul path in the real `uv run python3 control_room.py` loop against Elite + CrossOver.
