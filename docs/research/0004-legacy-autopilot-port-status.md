...
All three test items below are now tracked in `docs/plans/0004-runtime-diagnostics-dashboard.md`. The summaries here are kept as quick orientation, not as the authoritative scope.
...
- **"Stats for nerds" debugging dashboard.** Combine the two probes above plus other live signals (last journal event, current ship state, dispatched action history) into a `rich.live`-driven dashboard, modelled after YouTube's "stats for nerds" overlay. Should run alongside any autopilot routine or just on its own. Goal: when something feels laggy or wrong during a routine, one glance tells you whether the cause is capture, journal, or input dispatch.
- **~~Hotkey backend.~~** Parked 2026-05-27. See the Unverified list above for the reasoning. Revisit if and only if CLI-launched routines turn out to be friction in practice; the likely replacement is a menu-bar app, not a global hotkey listener.

Plan 0002 (CV scaffold) and plan 0003 (journal-driven routines) cover the two workstreams from the section above. The "Implication For Next Work" framing remains accurate; the plan docs are the actionable form.
